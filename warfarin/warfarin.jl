...
model3 = @model begin
    ...
    @random begin
        ...
        pd_η ~ MvLogNormal(
            log.([pop_e0, pop_emax, pop_c50, pop_tover]),
            pd_Ω,
        )
    end
...
[simobs(model, pop, coef(fpm)) for _ in 1:100]
...
sims2 = simobs(model, pop, coef(fpm), empirical_bayes(fpm))
# or
sims2 = simobs(fpm)
...
sims3 = simobs(bts_inf, pop, samples = 10)
# or
@time sims3 = [simobs(bts_inf, pop, samples = 1)[1] for _ in 1:1000]

# VPC from simulations
vpc_res_conc = vpc(sims3; observations = [:conc])
vpc_plot(
    vpc_res_conc;
    simquantile_medians = true,
    observations = false,
...
vpc_res_pca = vpc(sims3; observations = [:pca])
vpc_plot(
    vpc_res_pca;
    simquantile_medians = true,
    observations = false,
